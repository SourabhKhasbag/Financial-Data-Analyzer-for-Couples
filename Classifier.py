...
def startclassifer(path,outputPath):
    ...
    while id is not 100:                        #Continues till all 100 files have been parsed
        expense=0
        income=0
        filePath = path+"user-"+str(id) + exten
...
def fileinitiator(outputPath):
    '''
    Initiates the file and writes the header coloums to the file
    :return: None
    '''
    exten = ".csv"
    with open(outputPath+exten,'w') as csvfile:
...
def main():
    path=input("Enter filepath of data")
    outputPath=input("Enter the output file path")
    userList,userDict=startclassifer(path,outputPath)
    fileinitiator(outputPath)
